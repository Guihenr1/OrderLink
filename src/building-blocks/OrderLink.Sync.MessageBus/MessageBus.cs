...
using OrderLink.Sync.Core.Messages.Integration;
...
public class MessageBus : IMessageBus
{
    ...
    private void TryConnect()
    {
        ...
        policy.Execute(() =>
        {
            _bus = RabbitHutch.CreateBus(_connectionString, s =>
            {
                s.Register<IConsumerErrorStrategy>(_ => new AlwaysRequeueErrorStrategy());
                s.Register<IProduceConsumeInterceptor>(new LoggingInterceptor());
                s.Register<IConventions, AttributeBasedConventions>();
            });
    ...
    public sealed class AlwaysRequeueErrorStrategy : IConsumerErrorStrategy
    {
        ...
        public async Task<AckStrategy> HandleConsumerErrorAsync(ConsumerExecutionContext context, Exception exception, CancellationToken cancellationToken = default)
        {
            return await Task.FromResult(AckStrategies.NackWithRequeue);
        }

        public async Task<AckStrategy> HandleConsumerCancelledAsync(ConsumerExecutionContext context, CancellationToken cancellationToken = default)
        {
            return await Task.FromResult(AckStrategies.NackWithRequeue);
        }
    }
